...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def merge(self, mapping: torch.Tensor,) -> None:
        ...
        for q in range(8):
            ...
            for weight_name in weight_names:
                for idx in indices:
                    if idx == q:
                        w = getattr(self.experts[idx], weight_name).weight
                    else:
                        w = w + getattr(self.experts[idx], weight_name).weight
                w=w/(len(indices))
                w=torch.nn.Parameter(w)
                # print(self.layer_idx)
                # print(w.device)
                # print(getattr(self.experts[idx], weight_name).weight.device)
                if w.device != getattr(self.experts[idx], weight_name).weight.device:
                    w.to(getattr(self.experts[idx], weight_name).weight.device)
                getattr(self.experts[q], weight_name).weight=w
        ...
        for q in range(8):
            ...
            weight_names = ['w1', 'w2', 'w3']
            for weight_name in weight_names:
                w = getattr(self.experts[q], weight_name).weight
                w = w*0
                w = torch.nn.Parameter(w)
                getattr(self.experts[q], weight_name).weight=w
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # ADDED:
        # if len(self.skip)>0 and len(self.dom)>0 and len(self.skip) == len(self.dom):
        #     if self.flag==True:
        #         for idx in range(len(self.skip)):
        #             router_logits[:,self.skip[idx]]=torch.finfo(router_logits.dtype).min
        #             routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
        #     elif self.flag==False:
        #         for idx in range(len(self.skip)):
        #             routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
        #             routing_weights[:,self.dom[idx]]+=routing_weights[:,self.skip[idx]]
        #             routing_weights[:,self.skip[idx]]=0
        #     else:
        #         print('wtf')
        #         exit(0)
        # else:
        ...
        if self.mapping is not None:
            for q in range(8):
                indices = torch.where(self.mapping == q)[0]
                if len(indices)==0:
                    continue
                for idx in indices:
                    if idx == q:
        ...
        #     if self.layer_idx==3:
        #         exit()
        # if self.layer_idx == 0 and sequence_length != 0:
        #     print(selected_experts)
...
# class LMEvalMixtralForCausalLM(HFLM):
...
#                         )
#             self._model = MixtralForCausalLM.from_pretrained(
#                 pretrained,
#                 cache_dir=cache_dir,
#                 revision=revision,
#                 config=self._config,
